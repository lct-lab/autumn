...
inline void allocate_env_handle(SQLHENV& env, const bool pooling)
{
    if (env)
        return;
...
public:
    ...
    connection_impl(const string& dsn, const string& user, const string& pass, long timeout, const bool pooling)
        : dbc_(nullptr)
        , connected_(false)
        , transactions_(0)
        , rollback_(false)
    ...
    connection_impl(const string& connection_string, long timeout, const bool pooling)
        : dbc_(nullptr)
        , connected_(false)
        , transactions_(0)
        , rollback_(false)
    ...
    void allocate()
    {
        allocate_env_handle(env_, pooling_);
...
//        deallocate_handle(env_, SQL_HANDLE_ENV);
        ...
        void* event_handle = nullptr)
    {
        ...
        if (timeout != 0)
        {
            // Avoid to set the sqlTimeout to 0 (no sqlTimeout).
    ...
    connect(const string& connection_string, long timeout, void* event_handle = nullptr)
    {
        ...
        deallocate_handle(dbc_, SQL_HANDLE_DBC);//free
        allocate_dbc_handle(dbc_, env_);
...
HENV connection::connection_impl::env_ = nullptr;
...
public:
    ...
    {
        close();// cancel and free statement handler
        RETCODE rc;
    ...
    void timeout(long timeout)
    {
        ...
        // some drivers don't support sqlTimeout for statements,
        // so only raise the error if a non-default sqlTimeout was requested.
        ...
        statement& statement)
    {
        ...
        const RETCODE rc = just_execute_direct(conn, query, batch_operations, sqlTimeout, statement);
        if (rc == SQL_NO_DATA)
            return result();
#else
    ...
    {
#ifdef NANODBC_ENABLE_WORKAROUND_NODATA
        const RETCODE rc = just_execute(batch_operations, sqlTimeout, statement);
        if (rc == SQL_NO_DATA)
            return result();
#else
...
connection::connection(const string& dsn, const string& user, const string& pass, long timeout, bool pooling)
    : impl_(new connection_impl(dsn, user, pass, timeout, pooling))
{
}

connection::connection(const string& connection_string, long timeout, bool pooling)
    : impl_(new connection_impl(connection_string, timeout, pooling))
{
}
